...
function ImageCroper({ imageSrc, closeModal, updateProfile }) {
  ...
  async function handleCanvas() {
    try {
      const blob = await canvasOutput(imageRef.current, canvasRef.current, crop);
  ...
  return (
    <div className="min-h-screen absolute inset-0 flex justify-center items-center bg-[#000000d5]">
      <div className="bg-white flex flex-col items-center gap-5 p-5 rounded-xl">
        <a
          onClick={closeModal}
          className="text-[20px] px-5 py-3 absolute right-5 top-5 text-white bg-black rounded-full"
        >
