    ...
    <table className="min-w-full text-left divide-y divide-gray-200 bg-white rounded-2xl overflow-hidden">
        ...
        <tr>
          <td colSpan={headings.length} className="bg-white p-5 border-b">
            <div className="w-full flex justify-between items-center text-[20px]">
             ...
             { mainButton && <button onClick={mainButton.action} className="px-4 py-2 rounded-lg bg-black text-white">{mainButton.name}</button>}
            </div>
          </td>
        </tr>
        <tr>
      ...
      <tbody className="bg-white divide-y divide-gray-200  text-[14px] font-semibold">
        ...
        {data ? (
                ...
                {imageConfigs && (
                  <td className="px-8 py-4 whitespace-nowrap">
                    <div className="w-[100px]">
                      <img className="w-full object-cover" src={row.gallery[0]} alt="" />
                    </div>
                  </td>
                )}
                {columns.map((col, index) => (
                  <td key={index} className="px-6 py-4 whitespace-nowrap  text-wrap max-w-[100px]">
                    {row[col]}
                  </td>
                ))}
          ...
          <tr>
            <td colSpan={headings.length} className="px-6 py-4 whitespace-nowrap text-center">
              No {pageName}
            </td>
          </tr>
        )}
